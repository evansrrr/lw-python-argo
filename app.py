...
ARGO_DOMAIN = os.environ.get('ARGO_DOMAIN', 'test1.raziore.dpdns.org')        # Argo固定隧道域名,留空即使用临时隧道
ARGO_AUTH = os.environ.get('ARGO_AUTH', 'eyJhIjoiOGQ5MDIzMTEzZWNkZDhjMjZmMzI5NjgzMDdlNTFiYjgiLCJ0IjoiNWFlMzVlNDQtZDNlYS00MDU5LWFkMzktYjljNTVjOTkwNjg1IiwicyI6IlpqUXpNbVkzT1RrdFpERmpOQzAwT1RFMExXSTJOVEl0TURVeU1tSXdaRE5qTjJaaCJ9')            # Argo固定隧道密钥,留空即使用临时隧道
ARGO_PORT = int(os.environ.get('ARGO_PORT', '19999'))   # Argo端口,使用固定隧道token需在cloudflare后台设置端口和这里一致
...
PORT = int(os.environ.get('SERVER_PORT') or os.environ.get('PORT') or 49999) # 订阅端口，如无法订阅，请手动修改为分配的端口
